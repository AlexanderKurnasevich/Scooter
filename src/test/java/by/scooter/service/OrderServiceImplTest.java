...
class OrderServiceImplTest {
    ...
    @Test
    void updateOrder() {
        OrderDTO given = new OrderDTO();
        Order expected = new Order();
        when(mapper.map(given, Order.class)).thenReturn(expected);

        orderService.updateOrder(1L, given);

        verify(mapper, times(1)).map(given, Order.class);
        verify(orderDAO, times(1)).update(expected);
    }
